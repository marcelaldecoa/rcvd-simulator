...
> **Not yet verified against real iRacing.** The post-session path is exercised
> end to end against genuine iRacing-layout bytes, and the overlay is checked in
> a real Electron window. The *live* shared-memory path has never touched the
> actual sim. Treat your first live session as a shakedown.
...
| **Start here** | iRacing Telemetry | Live slip angles, the overlay, post-session identification and lap comparison |
